...
- (void)patchExecAndSignIfNeedWithCompletionHandler:(void(^)(bool success, NSString* errorInfo))completetionHandler progressHandler:(void(^)(NSProgress* progress))progressHandler forceSign:(BOOL)forceSign {
    ...
    // Validate CFBundleExecutable exists in Info.plist
    NSString *bundleExecutable = _infoPlist[@"CFBundleExecutable"];
    if (!bundleExecutable || bundleExecutable.length == 0) {
        [NSUserDefaults.standardUserDefaults removeObjectForKey:@"SigningInProgress"];
        completetionHandler(NO, @"CFBundleExecutable missing from Info.plist. The app bundle may be corrupted or improperly installed.");
        return;
    }
    
    NSString *execPath = [NSString stringWithFormat:@"%@/%@", appPath, bundleExecutable];
    ...
    if (needPatch || forceSign) {
        ...
        NSString *backupPath = [NSString stringWithFormat:@"%@/%@_LiveContainerPatchBackUp", appPath, bundleExecutable];
        NSError *err;
    ...
    NSString* executablePath = [appPath stringByAppendingPathComponent:bundleExecutable];
    if(!forceSign) {
