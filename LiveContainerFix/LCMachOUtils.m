...
NSString *LCParseMachO(const char *path, bool readOnly, LCParseMachOCallback callback) {
    // Validate path doesn't contain "(null)" which indicates missing CFBundleExecutable
    if (strstr(path, "(null)") != NULL) {
        return @"Invalid app path: CFBundleExecutable missing from Info.plist. Please reinstall the app.";
    }
    ...
    // Check if file is empty or too small to be a valid Mach-O
    if (s.st_size < sizeof(uint32_t)) {
        close(fd);
        return [NSString stringWithFormat:@"File too small or empty: %s", path];
    }
