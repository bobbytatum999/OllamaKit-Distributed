...
static NSString* invokeAppMain(NSString *selectedApp, NSString *selectedContainer, int argc, char *argv[]) {
    ...
    // Validate that the app has a valid executable
    NSString *bundleExecutable = appBundle.infoDictionary[@"CFBundleExecutable"];
    if (!bundleExecutable || bundleExecutable.length == 0) {
        return @"App is corrupted: CFBundleExecutable is missing from Info.plist. Please reinstall the app.";
    }
    
    NSString *executablePath = appBundle.executablePath;
    if (!executablePath || [executablePath containsString:@"(null)"]) {
        return @"App is corrupted: executable path is invalid. Please reinstall the app.";
    }
